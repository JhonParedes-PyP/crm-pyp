...
# 1. Add Edit Metas button
target_metas = '<h3 style="color: #003366; margin-bottom: 15px; margin-top: 0;">🎯 Recuperación y Metas por Cartera (Mes Actual)</h3>'
replacement_metas = """<h3 style="color: #003366; margin-bottom: 15px; margin-top: 0; display: flex; align-items: center;">
...
if target_metas in content:
    content = content.replace(target_metas, replacement_metas)

# 2. Add Revisar button
target_revisar = """                        <td style="padding: 10px; text-align: center;">
                            
                        </td>
                    </tr>"""
replacement_revisar = """                        <td style="padding: 10px; text-align: center;">
                            <a href="{% url 'registrar_gestion' deudor_id=p.id %}" target="_blank" style="background: #003366; color: white; padding: 4px 10px; text-decoration: none; border-radius: 4px; font-size: 12px;">Revisar</a>
                        </td>
                    </tr>"""

if target_revisar in content:
    content = content.replace(target_revisar, replacement_revisar)

# 3. Add Modal JS at the very end (before {% endblock %})
modal_js = """
...
if '{% endblock %}' in content:
    content = content.replace('{% endblock %}', modal_js)
